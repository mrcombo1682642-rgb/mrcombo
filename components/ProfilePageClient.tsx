...
export default function ProfilePageClient({ targetUsername }: { targetUsername: string }) {
  ...
  async function init() {
    ...
    const { data: visitData } = await supabase
      .from("profile_visits")
      .select("visitor_username, visited_at")
  ...
  async function uploadImage(file: File, type: "avatar" | "cover") {
    ...
    if (!updateError) {
      setProfile(prev => prev ? { ...prev, [column]: publicUrl } as Profile : prev);
    }
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", background: "#050a0f" }}>
        <Navbar />
        <div style={{ textAlign: "center", padding: "140px 0", color: "#4a7a94" }}>
          Loading profile...
        </div>
        <Footer />
      </div>
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#050a0f" }}>
        ...
        <div style={{ textAlign: "center", padding: "140px 16px", color: "#4a7a94" }}>
          ...
          <p style={{ fontSize: 14 }}>No profile exists with username &quot;{targetUsername}&quot;</p>
          <Link href="/" style={{ color: "#00b4d8", fontSize: 13, marginTop: 16, display: "inline-block" }}>
            ← Back to Home
          </Link>
        </div>
        <Footer />
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#050a0f", color: "#c8dde8", fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "80px 14px 60px" }}>
        ...
        {/* COVER BANNER */}
        <div style={{
          position: "relative", height: 200, borderRadius: "12px 12px 0 0",
          overflow: "hidden", background: profile.cover_url
            ? `url(${profile.cover_url}) center/cover no-repeat`
            : "linear-gradient(135deg, #1a0840, #0d1f2d, #001a2d)",
        ...
        {/* PROFILE HEADER */}
        <div style={{
          background: "#0a1520", border: "1px solid #1a2535", borderTop: "none",
          padding: "0 24px 20px", display: "flex", alignItems: "flex-end", gap: 18,
          flexWrap: "wrap", position: "relative",
        }}>
          <div style={{ marginTop: -50, position: "relative", flexShrink: 0 }}>
            <div style={{
              width: 100, height: 100, borderRadius: 14,
              border: `3px solid ${badge.color}`, overflow: "hidden",
              background: "#1a2535", display: "flex", alignItems: "center",
              justifyContent: "center", fontSize: 36, fontWeight: 700, color: badge.color,
            ...
            <div style={{
              ...
              background: isOnline ? "#22c55e" : "#4a5568",
          ...
          <div style={{ flex: 1, paddingTop: 12, minWidth: 200 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              ...
              <span style={{
                ...
                background: `${badge.color}1a`, border: `1px solid ${badge.color}44`,
            ...
            {profile.bio && (
              <p style={{ fontSize: 13, color: "#9ab0bf", marginTop: 8, maxWidth: 500 }}>
                {profile.bio}
              </p>
            )}
          </div>
            ...
            {isOwner ? (
              <>
                <button onClick={() => setEditOpen(true)} style={{
                  ...
                  padding: "9px 18px", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer",
                }}>
                  ✏️ Edit Profile
                </button>
                <button onClick={handleLogout} style={{
                  background: "#1a2535", border: "1px solid #2a3545", borderRadius: 7,
                  padding: "9px 18px", color: "#c8dde8", fontSize: 13, fontWeight: 600, cursor: "pointer",
                }}>
                  Logout
                </button>
              </>
              ...
              currentUserId && (
                <Link href="/" style={{
                  background: "#0d2030", border: "1px solid #1a3042", borderRadius: 7,
                  padding: "9px 18px", color: "#00b4d8", fontSize: 13, fontWeight: 700,
        ...
        <div style={{ display: "grid", gridTemplateColumns: "280px 1fr 260px", gap: 16, marginTop: 16 }} className="profile-grid">
          ...
          <div>
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <BoxBody>
                {profile.signature ? (
                  <p style={{ fontSize: 13, color: "#9ab0bf", lineHeight: 1.7, margin: 0, whiteSpace: "pre-wrap" }}>
                    {profile.signature}
                  </p>
                ) : (
                      ...
                      <Link key={i} href={`/profile/${v.visitor_username}`} style={{ textDecoration: "none" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: 12.5, color: "#c8dde8", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {v.visitor_username}
                            </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              ...
              <span style={{ fontWeight: 700, fontSize: 15, color: "#fff" }}>✏️ Edit Profile</span>
            ...
            <div style={{ padding: 20 }}>
              <Field label="BIO">
                <textarea value={editBio} onChange={e => setEditBio(e.target.value)}
                  placeholder="Tell others about yourself..." rows={3} style={textareaStyle} />
              </Field>

              <Field label="SIGNATURE">
                <textarea value={editSignature} onChange={e => setEditSignature(e.target.value)}
                  placeholder="Your forum signature (shown under your name)..." rows={3} style={textareaStyle} />
              </Field>
              ...
              <Field label="TELEGRAM ID">
                <input value={editTelegram} onChange={e => setEditTelegram(e.target.value)}
              ...
              {saveMsg && (
                <div style={{ fontSize: 12, color: saveMsg === "Saved!" ? "#22c55e" : "#ef4444", marginBottom: 10 }}>
                  {saveMsg}
                </div>
              )}

              <button onClick={handleSaveEdit} disabled={saving} style={{
                width: "100%", background: "#6c63ff", border: "none", borderRadius: 8,
...
function InfoRow({ label, value, valueColor }: { label: string; value: string; valueColor?: string }) {
  return (
    <div style={{
      ...
      padding: "6px 0", borderBottom: "1px solid #0d1c28", fontSize: 12.5,
    }}>
